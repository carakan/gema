...
class Marca < ActiveRecord::Base
  ...
  def self.paginacion(options = {})
    ...
    @marcas = Marca.paginate(:per_page => 10, :page => options[:page], :conditions => ["marcas.nombre LIKE ? OR marcas.numero_solicitud LIKE ?", "%#{search}%", "%#{search}%"], :include => [:tipo_signo, :titulares], :order => "#{order} #{direction}")
  end
