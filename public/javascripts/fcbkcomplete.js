 ...
 * ajax - object with two parametrs a) url to fetch json obj b) use cache
...
jQuery.facebooklist = function(elem, list, complete, ajax, height, filter, newel){
    ...
    var addItem = function(item, preadded){
        ...
        if (!preadded) {
            ...
            holder.removeChild($(holder).find('.bit-input')[0]);
            addInput();
        }
        feed.hide();
    }
    ...
    var addInput = function(){
        ...
        $(holder).click(function(){
          ...
          if (feed.length && $(input).val().length) {
            ...
            feed.css('height','0px');
            $(complete).find('.default:first').show();
          }
        });
    ...
    var feed = $(complete).find('.feed:first');
    var cache = {};
    var counter = 0;
	var nowFocusOn;
